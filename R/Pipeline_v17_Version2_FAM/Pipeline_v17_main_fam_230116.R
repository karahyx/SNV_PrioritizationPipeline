...
#          compound heterozygotes in a given child.
...
# args[3]: family variant input file path
# args[4]: child genome name used at columns
# args[5]: sub-path and filename of pedigree file
# args[6]: sub-path and filename of stats visualization Rmd file
# args[7]: output sub-path

# args <- character(7)
# args[1] <- "/Users/karahan/Desktop/PrioritizationPipeline/"
# args[2] <- "script/R/pipeline/v230116/Pipeline_v17_funcs_fam_230116.R"
...
# args[4] <- "NA24385_b"
# args[6] <- "script/R/pipeline/stats/Pipeline_stats.Rmd"
# args[7] <- "/Users/karahan/Desktop/PrioritizationPipeline/tests"

root.path <- args[1]
fun.path_filename <- paste (root.path, args[2], sep = "")
input_fam_var.path_filename  <- args[3]
input_child_genome.name      <- args[4]
input_pedigree.path_filename <- args[5]
input_stats_vis.path_filename <- paste (root.path, args[6], sep = "")
output_prefix.path_filename <- paste (root.path, args[7], "/", input_child_genome.name, sep = "")

alt_input_child_genome.name <- paste0 (input_child_genome.name, ":")
child_column_recover.name <- paste0 (input_child_genome.name, "*:")

cat ("\n")
cat ("R functions path and filename ");         cat (fun.path_filename);             cat ("\n")
cat ("Family variant data path and filename "); cat (input_fam_var.path_filename);   cat ("\n")
cat ("Child genome name ");                     cat (input_child_genome.name);       cat ("\n")
cat ("Pedigree path and filename ");            cat (input_pedigree.path_filename);  cat ("\n")
...
# pedigree <- data.table::fread(input_pedigree.path_filename)
pedigree <- data.frame(rep("AZ", 3),
                       c("NA24149_b", "NA24143_b", input_child_genome.name),
...
child <- pedigree %>% subset(Person == input_child_genome.name)
parents <- pedigree %>% subset(Person == child$Father | Person == child$Mother)

father_genome.name <- parents %>% subset(Sex == "male") %>% pull(Person)
mother_genome.name <- parents %>% subset(Sex == "female") %>% pull(Person)
father_column.name <- paste0(father_genome.name, ":")
mother_column.name <- paste0(mother_genome.name, ":")

fam_dat <- data.table(name = c("child", "father", "mother"), 
                      id = c(input_child_genome.name, father_genome.name, mother_genome.name),
                      sex = c(child$Sex, "male", "female"),
                      gt = character(3))

# 2.3. Import variant data
v_full.temp.df <- data.table::fread(input_fam_var.path_filename, data.table = F, na.strings = c("NA", "."))
check_samples_exist(v_full.temp.df, alt_input_child_genome.name, father_column.name, mother_column.name)
v_full.temp.df <- subset(v_full.temp.df, select = -c(DP, cg_freq_max)) # DP is removed to avoid conflict after removing "child_sample_name:" for processing

# 2.4. Re-format column names
child_cols <- grep(alt_input_child_genome.name, names(v_full.temp.df))
names(v_full.temp.df) <- gsub(alt_input_child_genome.name, "", names(v_full.temp.df)) # remove "sample_name" from the child's columns
...
# 2.5.1. Add some fields for allele frequency
...
# 2.5.2. Add some fields for gene constraint
...
v_full_r05.df <- get_rare05_variants(v_full.df, output_prefix.path_filename)
...
# 2.10. Process the results before outputting
v_full_r05.df <- change_tier_levels(v_full_r05.df) # change tier levels from 0, 1 and 2 to "-", "Low" and "High"
names(v_full_r05.df)[child_cols + 1] <- paste0(child_column_recover.name, names(v_full_r05.df)[child_cols + 1]) # add child's sample_name back to columns
