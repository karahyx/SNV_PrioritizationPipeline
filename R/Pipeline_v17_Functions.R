...
add_coding_lof_tag <- function(data, eff_lof.chv) {
  ...
  lof_tier1_var <- with(data, which(F_DamageType == "LOF" & str_count(dbsnp_region) > 1))
  lof_tier2_var <- with(data, which(F_DamageType == "LOF" & 
                                   (is.na(str_count(dbsnp_region)) | str_count(dbsnp_region) == 1)))
  data$F_DamageTier[lof_tier1_var] <- 1
  data$F_DamageTier[lof_tier2_var] <- 2
...
add_missense_tag <- function(data, sift_cutoff, polyphen_cutoff, ma_cutoff,
  ...
  if (missense_t1_cutoff > missense_t2_cutoff) {
    stop("missense_tier1_cutoff should be <= missense_tier2_cutoff, otherwise 
         F_DamageType would not be correctly labelled as 'Missense'")
  }
  
  missense_mx <- matrix(data = 0, nrow = nrow(data), ncol = 7)
  ...
  missense_mx[, 4] <- with(data, as.numeric(phylopMam_avg  >= phylopMam_missense_cutoff | phylopVert100_avg >= phylopVert_missense_cutoff)) 
  missense_mx[, 5] <- with(data, as.numeric(CADD_phred     >= CADD_phred_missense_cutoff))
  missense_mx[, 6] <- with(data, as.numeric(REVEL_score    >= REVEL_cutoff)) 
  missense_mx[, 7] <- with(data, as.numeric(MPC_score      >= MPC_cutoff))
  ...
  missense_tier1_var <- with(data, which(F_Coding == "Coding" & 
                                           (effect_priority %in% eff_missn.chv &  
                                              (missense_score >= missense_t1_cutoff))))
  
  missense_tier2_var <- with(data, which(F_Coding == "Coding" & 
                                           (effect_priority %in% eff_missn.chv & 
                                              (missense_score >= missense_t2_cutoff))))
...
# 3.5.1. and 3.5.2. HPO / CGD dominant
